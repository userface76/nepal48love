...
/** 중복 없는 무작위 추천코드 (이름을 코드로 쓸 수 없을 때의 대비책) */
...
/** 이미 쓰이고 있는 코드인가 — 영문 대소문자는 같은 것으로 봅니다 */
async function codeTaken(db, code) {
  return db.prepare('SELECT 1 FROM participants WHERE code = ? COLLATE NOCASE').bind(code).first();
}

/** 이름을 추천코드 모양으로 다듬습니다 — 공백 제거, 한글·영문·숫자만 남김 */
export function nameToCodeBase(name) {
  return String(name ?? '')
    .replace(/\s+/g, '')
...
/** 사용자가 입력한 추천코드 정리 — 한글 이름 코드를 허용합니다 */
export function normalizeCode(v) {
  return nameToCodeBase(v).slice(0, 24);
}

/**
 * 이름 기반 추천코드 발급 — 홍길동 · 홍길동2 · 홍길동3 …
 * 이름에서 쓸 글자가 하나도 없으면 무작위 코드로 돌아갑니다.
 */
export async function issueNameCode(db, name) {
  const base = nameToCodeBase(name);
  if (!base) return issueUniqueCode(db);
  for (let n = 1; n <= 999; n++) {
    const code = n === 1 ? base : `${base}${n}`;
    if (!(await codeTaken(db, code))) return code;
  }
  return issueUniqueCode(db);
}
